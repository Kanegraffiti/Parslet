...
try:
    ...
    pydot = None  # Assign to None to satisfy linters if pydot is None in type hints.
...
class PydotImportError(ImportError):
    ...
    def __init__(
        self,
        message: str = "pydot library is not installed. Please install it to use DOT/PNG export features (e.g., 'pip install pydot').",
    ):
...
class GraphvizExecutableNotFoundError(RuntimeError):
    """
    Custom exception raised when Graphviz executable (e.g., 'dot') is not found by `pydot`.

    This error indicates that while `pydot` might be installed, the underlying
    Graphviz tools (which `pydot` calls to render images) are not in the system's
    PATH or are not installed.
    """

    def __init__(
        self,
        message: str = "Graphviz executables (like 'dot') not found in system PATH. Graphviz is required for PNG export. Please install Graphviz and ensure it's in your PATH.",
    ):
        super().__init__(message)
...
def dag_to_pydot(dag: "DAG") -> "Optional[pydot.Dot]":
    ...
    Returns:
        Optional[pydot.Dot]: A `pydot.Dot` object representing the graph if `pydot`
                             is available. Returns `None` if `pydot` is not installed
                             (though it typically raises PydotImportError first).
                             The type hint reflects that `pydot` itself might be `None`
                             if not imported.

    Raises:
        PydotImportError: If the `pydot` library is not installed.
        TypeError: If the graph object within the DAG is not a NetworkX DiGraph.
        RuntimeError: If `nx.drawing.nx_pydot.to_pydot` fails for other reasons.
    """
    if not PYDOT_AVAILABLE or pydot is None:
        # This check ensures pydot was imported successfully at the module level.
        raise PydotImportError()

    if not isinstance(dag.graph, nx.DiGraph):
        raise TypeError(
            f"DAG's internal graph must be a NetworkX DiGraph. Got: {type(dag.graph)}"
        )

    # Convert the NetworkX graph (dag.graph) to a pydot graph.
    # Custom node attributes (shape, color, etc.) or edge attributes can be
    # defined in the NetworkX graph nodes/edges and would be translated by to_pydot.
    # For Parslet, node labels are typically their task IDs.
    try:
...
def save_dag_to_png(dag: "DAG", output_path: str) -> None:
    ...
    installation (specifically, the 'dot' executable must be in the system PATH).
    ...
    Raises:
        PydotImportError: If `pydot` is not installed (propagated from `dag_to_pydot`).
        GraphvizExecutableNotFoundError: If Graphviz 'dot' executable is not found by `pydot`.
    ...
    try:
        # The write_png method of pydot.Dot calls the Graphviz 'dot' executable
        # to render the graph and save it as a PNG.
        ...
        # This can occur if 'dot' executable is not found or if the output_path is invalid.
        # We try to distinguish based on the error message.
        ...
        raise RuntimeError(
            f"Failed to write PNG file at '{output_path}': {e}. Check path and permissions."
        ) from e
        ...
        raise RuntimeError(
            f"Failed to create or write PNG file at '{output_path}': {type(e).__name__} - {e}"
        ) from e
...
if __name__ == "__main__":
    ...
    logger = logging.getLogger(__name__)
    # Attach logger to this module for save_dag_to_png to use if needed (by making it part of a class or passing)
    # For this standalone script test, we'd ideally pass a logger to save_dag_to_png or handle logging internally.
    # Let's simulate by setting a module-level logger for the test.
    # Note: The `save_dag_to_png` function would need modification to use `logger` directly without `self`.
    ...
    try:
        ...
        if PYDOT_AVAILABLE:
            ...
            try:
                ...
                try:
                    # Modify save_dag_to_png to accept a logger for this test, or use a global one.
                    ...
                    logger.info(
                        f"DAG visualization attempt to save to {png_output_path}."
                    )
                    logger.info(
                        "If Graphviz 'dot' is installed and in PATH, %s should be created.",
                        png_output_path,
                    )
                except GraphvizExecutableNotFoundError as gne:
                    logger.error(
                        "PNG EXPORT FAILED: Graphviz 'dot' executable not found. PNG not created."
                    )
                    logger.error("Details: %s", gne)
                    logger.error(
                        "Please install Graphviz (from https://graphviz.org/download/) and ensure 'dot' is in your system PATH."
                    )
                except (
                    PydotImportError
                ...
                logger.error(
                    f"Error during dummy DAG creation or export tests: {e}",
                    exc_info=True,
                )
        else:
        ...
        logger.error(
            "Failed to import Parslet core components for testing exporter. Ensure PYTHONPATH is set correctly if running directly."
        )
